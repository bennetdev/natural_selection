; wenn die Menschen zu schnell sind verringert sich die chance dass mein mensch 2 foods sammelt dadurch
; kann sich weniger reproduziert werden und die Menge wird weniger
; um das auszugleichen passt sich der durchschnittsspeed in der Population an
; show 0.7 / 300


; observer: 0.002333333333333333
; observer> show 0.7 / 300
; observer: 0.002333333333333333
; observer> show 300 / 0.7
; observer: 428.5714285714286
; observer> show 428 / 18
; observer: 23.77777777777778
  ...
  ask human[
    ifelse energy > 0 [
      ...
      [
        ...
        ; TODO: should only move from edges away
        rt random 50
        lt random 50
        fd speed
        ; reduce energy
        set energy (energy - speed * speed-cost-factor)
        ; try to eat on current patch
        try-eat
      ]
    ]
    ; if energy is empty -> human die
    [
      if foods-found = 0 [die]
    ]
  ]
  ; reset state if food is left or all human have eaten enough (1x)
  if count foods = 0 or all? human [foods-found >= 1] [
    ; add speed to avg-speed-forever
    set avg-speed-forever lput (mean [speed] of human) avg-speed-forever

    kill-human reset-human reset-foods set day (day + 1)
    ; plot with days on x axis
    set-current-plot "pupolations"
    set-current-plot-pen "human-pop"
    plotxy day (count human)
  ]
end

to try-eat
  ; get all foods on current patch
  ; TODO: should only be one at max
  let all-foods-found foods-here
...
to reproduce-human
  ; reproduce all human with 2 foods
  let add-human-count (count (human with [foods-found = 2]))
  ...
  ; add to counter
  set lived (lived + add-human-count)
end
...
to reset-human
  ...
  ask human[
    ...
    ; reset color
    let col yellow
    ;set col lput (speed * 100) col
    set color col
    ; set age
    set age (age + 1);

    let one_of one-of [-16 16]
    let rand (random 32 - 16)
    ; place human either on top/bottom or left/right side
    ifelse random 2 = 1 [
      setxy one_of rand
      ifelse one_of = 16 [set heading 270][set heading 90]
    ] [
      setxy rand one_of
      ifelse one_of = 16 [set heading 180][set heading 0]
    ]
  ]
end

to reset-foods
  create-foods (init-foods - count foods)
  ask foods[
...
"set-histogram-num-bars 10" ""
PENS
"default" 1.0 1 -10899396 true "" "histogram avg-speed-forever"
